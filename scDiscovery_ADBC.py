...
from data_split_multiomics import PairedMultiOmicsDataset
from torch.utils.data import DataLoader
import random
from collections import defaultdict
from scDiscovery_step1_known import evaluate_model
import scanpy as sc
...
@torch.no_grad()
def visualize_latent_tsne_joint(
        E_rna,
        E_atac,
        dataloader_rna,
        dataloader_atac,
    ...
    # --------------------------------------------------
    Z_rna, Y_rna, X_rna = collect_latent(E_rna, dataloader_rna, "RNA")
    Z_atac, Y_atac, X_atac = collect_latent(E_atac, dataloader_atac, "ATAC")

    return Z_rna, Z_atac, Y_rna, Y_atac, X_rna, X_atac

    # n = min(len(Z_rna), len(Z_atac))
    # Z_rna = Z_rna[:n]
    # Z_atac = Z_atac[:n]
    #
    # Z_all = torch.cat([Z_rna, Z_atac], dim=0).numpy()
    # y_mod = np.array([0] * n + [1] * n)  # 0=RNA, 1=ATAC
    #
    # # --------------------------------------------------
    # # t-SNE (joint)
    # # --------------------------------------------------
    ...
    # plt.title(f"Joint Latent t-SNE ({title_suffix})")
    # plt.legend(markerscale=2, fontsize=9)
    # plt.tight_layout()
    #
    # fig_path = os.path.join(
    #     save_dir, f"joint_latent_tsne_{title_suffix}.png"
    # )
...
def filter_ood_data_with_multiomics(E_rna, E_atac, Classifier, paired_train_loader, paired_mixed_loader, paired_test_ood_loader, filtered_paired_ood_loader, class_number, pre_threshold, num_iters, device, iter, recall_level=0.95):
    id_scores, _, _, _, _, _ = get_ood_scores_with_multiomics(E_rna, E_atac, Classifier, paired_train_loader, device)

    mixed_paired_scores, rna_data, atac_data, paired_label, rna_raw_data, atac_raw_data = get_ood_scores_with_multiomics(E_rna, E_atac, Classifier, paired_mixed_loader, device)
    ...
    mask = mixed_paired_scores < threshold
    filtered_rna_data = rna_data[mask]  # 筛选后的数据
    filtered_atac_data = atac_data[mask]  # 筛选后的数据

    filtered_raw_rna_data = rna_raw_data[mask]
    filtered_raw_atac_data = atac_raw_data[mask]

    if len(filtered_rna_data) == 0:
        # threshold += 0.1

        mask = mixed_paired_scores < threshold+0.1
    ...
    filtered_paired_ood_label = 1000*torch.ones(len(filtered_rna_data), dtype=torch.long)

    # filtered_omics1_dataset = BuildDataset(filtered_raw_data1, filtered_data1, filtered_label1)

    paired_filtered_dataset = PairedMultiOmicsDataset(
        rna_raw=filtered_raw_rna_data,
        rna=filtered_rna_data,
        atac_raw=filtered_raw_atac_data,
        atac=filtered_atac_data,
        labels=filtered_paired_ood_label,  # paired_label
    )

    filtered_paired_ood_loader = DataLoader(
        dataset=paired_filtered_dataset, batch_size=128, shuffle=True, drop_last=False
    )

    print(f'Threshold in This Iter:{threshold}')

    return filtered_paired_ood_loader, threshold


def train_distinguish_id_ood(E_rna, E_atac, Classifier, optimizer_E_rna, optimizer_E_atac, optimizer_fc, scheduler_E_rna, scheduler_E_atac, scheduler_fc, omics1_train_loader, omics1_test_id_loader, omics1_test_ood_loader, omics2_train_loader, omics2_test_id_loader, omics2_test_ood_loader, omics1_mixed_loader, omics2_mixed_loader, class_number, num_iters, num_epochs, recall_level):
    filtered_omics1_ood_loader = None
    filtered_omics2_ood_loader = None
    threshold = None

    final_threshold = evaluate_ood_detection(E_rna, E_atac, Classifier, omics1_test_id_loader, omics2_test_id_loader,
                                             omics1_test_ood_loader, omics2_test_ood_loader, recall_level, device)

    for i in range(num_iters):
        print(f'Iteration {i + 1}')
        if i == 0:
            filtered_omics1_ood_loader, filtered_omics2_ood_loader = init_filter(E_rna, E_atac, omics1_train_loader, omics2_train_loader, omics1_mixed_loader,
                        omics2_mixed_loader, omics1_test_ood_loader, omics2_test_ood_loader)
        else:
            filtered_omics1_ood_loader, filtered_omics2_ood_loader, threshold = filter_ood_data(E_rna, E_atac, Classifier,
                                                                                     omics1_train_loader,
                                                                                     omics2_train_loader,
                                                                                     omics1_mixed_loader,
                                                                                     omics2_mixed_loader,
                                                                                     omics1_test_ood_loader,
                                                                                     omics2_test_ood_loader,
                                                                                     filtered_omics1_ood_loader,
                                                                                     filtered_omics2_ood_loader,
                                                                                     class_number,
                                                                                     threshold,
                                                                                     num_iters,
                                                                                     iter=i,
        ...
        for epoch in range(epochs):
            ...
            E_atac.train()
            Classifier.train()
            for (_, x_id_1, y_1), (_, x_id_2, y_2), (_, x_ood_1, _), (_, x_ood_2, _) in zip(omics1_train_loader, omics2_train_loader, filtered_omics1_ood_loader, filtered_omics2_ood_loader):
                x_id_1, x_id_2, x_ood_1, x_ood_2 = x_id_1.cuda(), x_id_2.cuda(), x_ood_1.cuda(), x_ood_2.cuda()
                y_1, y_2 = y_1.cuda(), y_2.cuda()
                y = torch.cat((y_1, y_2), 0)

                x_1 = torch.cat((x_id_1, x_ood_1), 0)
                x_2 = torch.cat((x_id_2, x_ood_2), 0)
                ...
                logits_2 = logits[len(x_1):]

                logits_id_1 = logits_1[:len(x_id_1)]
                logits_id_2 = logits_2[:len(x_id_2)]
                logits_id = torch.cat((logits_id_1, logits_id_2), 0)

                logits_ood_1 = logits_1[len(x_id_1):]
                logits_ood_2 = logits_2[len(x_id_2):]
                logits_ood = torch.cat((logits_ood_1, logits_ood_2), 0)

                l_ce = F.cross_entropy(logits_id, y)

                l_oe = 0.5 * -(logits_ood.mean(1) - torch.logsumexp(logits_ood, dim=1)).mean()
            ...
            final_threshold = evaluate_ood_detection(E_rna, E_atac, Classifier, omics1_test_id_loader, omics2_test_id_loader,
                                   omics1_test_ood_loader, omics2_test_ood_loader, recall_level=0.95)
    filtered_omics1_ood_loader, filtered_omics2_ood_loader, filtered_omics1_id_loader, filtered_omics2_id_loader = get_detected_ood_data(E_rna, E_atac, Classifier, omics1_mixed_loader, omics2_mixed_loader, final_threshold)

    return E_rna, E_atac, Classifier, filtered_omics1_ood_loader, filtered_omics2_ood_loader, filtered_omics1_id_loader, filtered_omics2_id_loader


def train_distinguish_id_ood_with_single_omics(E_rna, Classifier, optimizer_E_rna, optimizer_fc, scheduler_E_rna, scheduler_fc, omics1_train_loader, omics1_test_id_loader, omics1_test_ood_loader, omics1_mixed_loader, class_number, num_iters, num_epochs, recall_level, threshold, device):
    filtered_omics1_ood_loader = None
    # threshold = None
    ...
    for i in range(num_iters):
        print(f'Iteration {i + 1}')
        if i == 0:
            # filtered_omics1_ood_loader = init_filter_with_single_omics(E_rna, omics1_train_loader, omics1_mixed_loader, omics1_test_ood_loader)
        ...
        # threshold -= 0.2
    filtered_omics1_ood_loader, filtered_omics1_id_loader = get_detected_ood_data_with_single_omics(E_rna, Classifier, omics1_mixed_loader, threshold, device=device)  # final_threshold

    return E_rna, Classifier, filtered_omics1_ood_loader, filtered_omics1_id_loader


def train_distinguish_id_ood_with_multiomics(E_rna, E_atac, Classifier, optimizer_E_rna, optimizer_E_atac, optimizer_fc, scheduler_E_rna, scheduler_E_atac, scheduler_fc, paired_train_loader, paired_test_id_loader, paired_test_ood_loader, paired_mixed_loader, class_number, num_iters, num_epochs, recall_level, threshold, device):
    filtered_paired_ood_loader = None
    # threshold = None

    if paired_test_id_loader is None:
        paired_test_id_loader = paired_train_loader

    final_threshold = evaluate_ood_detection_with_multiomics(E_rna, E_atac, Classifier, paired_test_id_loader, paired_test_ood_loader, recall_level, device)
    ...
    for i in range(num_iters):
        ...
        if i == 0:
            # filtered_omics1_ood_loader = init_filter_with_single_omics(E_rna, omics1_train_loader, omics1_mixed_loader, omics1_test_ood_loader)
            filtered_paired_ood_loader, threshold = filter_ood_data_with_multiomics(E_rna, E_atac, Classifier,
                                                                                      paired_train_loader,
                                                                                      paired_mixed_loader,
                                                                                      paired_test_ood_loader,
                                                                                      filtered_paired_ood_loader,
                                                                                      class_number,
                                                                                      threshold,
                                                                                      num_iters,
                                                                                      device,
        ...
        for epoch in range(epochs):
            ...
            E_atac.train()
            Classifier.train()
            for (x_raw_rna_id, x_rna_id, x_raw_atac_id, x_atac_id, labels_id), (x_raw_rna_ood, x_rna_ood, x_raw_atac_ood, x_atac_ood, labels_ood) in zip(paired_train_loader, filtered_paired_ood_loader):
                x_rna_id, x_atac_id, x_rna_ood, x_atac_ood = x_rna_id.to(device), x_atac_id.to(device), x_rna_ood.to(device), x_atac_ood.to(device)
                labels_id = labels_id.to(device)

                x_rna = torch.cat((x_rna_id, x_rna_ood), 0)
                x_atac = torch.cat((x_atac_id, x_atac_ood), 0)

                z_rna, _, _ = E_rna(x_rna)
                z_atac, _, _ = E_atac(x_atac)

                z = torch.cat((z_rna, z_atac), 0)  # [x_rna_id, x_rna_ood, x_atac_id, x_atac_ood]

                logits = Classifier(z)

                rna_len = len(x_rna_id) + len(x_rna_ood)

                logits_rna_id = logits[:len(x_rna_id)]
                logits_atac_id = logits[rna_len:rna_len+len(x_atac_id)]

                logits_rna_ood = logits[len(x_rna_id):rna_len]
                logits_atac_ood = logits[rna_len+len(x_atac_id):]

                logits_id = (7*logits_rna_id + 3*logits_atac_id) / 10

                logits_ood = (7*logits_rna_ood + 3*logits_atac_ood) / 10


                l_ce = F.cross_entropy(logits_id, labels_id)
        ...
        # threshold -= 0.2
    filtered_paired_ood_loader, filtered_paired_id_loader = get_detected_ood_data_with_multiomics(E_rna, E_atac, Classifier, paired_mixed_loader, final_threshold, device)

    return E_rna, E_atac, Classifier, filtered_paired_ood_loader, filtered_paired_id_loader
...
def get_detected_ood_data_with_single_omics(E_rna, Classifier, omics1_mixed_loader, final_threshold, device):
    E_rna.eval()
    Classifier.eval()

    mixed_omics1_scores, mixed_omics1_data, mixed_omics1_label, mixed_omics1_raw_data = get_ood_scores_with_single_omics(E_rna,
                                                                                                    Classifier,
    ...
    return filtered_omics1_ood_loader, filtered_omics1_id_loader


def get_detected_ood_data_with_multiomics(E_rna, E_atac, Classifier, paired_mixed_loader, final_threshold, device):
    E_rna.eval()
    E_atac.eval()
    Classifier.eval()

    mixed_paired_scores, rna_data, atac_data, paired_label, rna_raw_data, atac_raw_data = get_ood_scores_with_multiomics(
        E_rna, E_atac, Classifier, paired_mixed_loader, device)

    mask_ood = mixed_paired_scores < final_threshold

    # OOD
    filtered_ood_label = paired_label[mask_ood]  # 筛选后的数据

    filtered_rna_ood_data = rna_data[mask_ood]  # 筛选后的数据
    filtered_rna_ood_raw_data = rna_raw_data[mask_ood]

    filtered_atac_ood_data = atac_data[mask_ood]  # 筛选后的数据
    filtered_atac_ood_raw_data = atac_raw_data[mask_ood]

    # ID
    filtered_id_label = paired_label[~mask_ood]  # 筛选后的数据

    filtered_rna_id_data = rna_data[~mask_ood]  # 筛选后的数据
    filtered_rna_id_raw_data = rna_raw_data[~mask_ood]

    filtered_atac_id_data = atac_data[~mask_ood]  # 筛选后的数据
    filtered_atac_id_raw_data = atac_raw_data[~mask_ood]

    # Dataset
    paired_filtered_ood_dataset = PairedMultiOmicsDataset(
        rna_raw=filtered_rna_ood_raw_data,
        rna=filtered_rna_ood_data,
        atac_raw=filtered_atac_ood_raw_data,
        atac=filtered_atac_ood_data,
        labels=filtered_ood_label,  # paired_label
    )

    paired_filtered_id_dataset = PairedMultiOmicsDataset(
        rna_raw=filtered_rna_id_raw_data,
        rna=filtered_rna_id_data,
        atac_raw=filtered_atac_id_raw_data,
        atac=filtered_atac_id_data,
        labels=filtered_id_label,  # paired_label
    )

    # Dataloader
    filtered_paired_ood_loader = DataLoader(
        dataset=paired_filtered_ood_dataset, batch_size=128, shuffle=True, drop_last=False
    )

    filtered_paired_id_loader = DataLoader(
        dataset=paired_filtered_id_dataset, batch_size=128, shuffle=True, drop_last=False
    )

    return filtered_paired_ood_loader, filtered_paired_id_loader
...
def evaluate_ood_detection_with_multiomics(E_rna, E_atac, Classifier, paired_test_id_loader, paired_test_ood_loader, recall_level, device):
    id_scores, _, _, _, _, _ = get_ood_scores_with_multiomics(E_rna, E_atac, Classifier, paired_test_id_loader, device)
    ood_scores, _, _, _, _, _ = get_ood_scores_with_multiomics(E_rna, E_atac, Classifier, paired_test_ood_loader, device)
